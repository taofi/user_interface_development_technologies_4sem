...
async function fetchSeries(authToken: string): Promise<any | undefined> {
    const moviesUrl = "https://api4.thetvdb.com/v4/series";
...
async function fetchMovies(authToken: string): Promise<any | undefined> {
    const moviesUrl = "https://api4.thetvdb.com/v4/movies";
...
async function loadMovies() {
    ...
    if (authToken) {
        const moviesData = await fetchMovies(authToken);
        if(moviesData) {
...
async function loadSeries() {
    if(!authToken)
        authToken = await getAuthToken();
    if (authToken) {
        const seriesData = await fetchSeries(authToken);
        if(seriesData) {
